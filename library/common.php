...
function getRanking($det, $max){

			$high = $max * 0.51;
			$ave = $high * 0.52;
			$low = $ave  * 0.55;
			echo "HIGH - " .$high . " DET - ". $det;
			if( $det >= $high){
				return "Strongly";
			}else if( $det < $high && ( $det >= $ave || $det > $low)){
				return "Mildly";
			}else{
				return "Niether";
			}
...
function getDeterminant($ave){
			...
			if($high<$ave){
				return 5;
			}else if($high>$ave && $above<$ave){
				return 4;
			}else if($above>$ave && ($average<=$ave||$average>=$ave)&& $ave>$below){
				return 3;
			}else if($average>$ave && $ave>$below){
				return 2;
			}else if($ave<$below){
				return 1;
			}else{
...
{
	if (!isset($_SESSION['dss_error'])) {
		$_SESSION['dss_error'] = array();
	}
	
	$_SESSION['dss_error'][] = $errorMessage;
...
*/
function createGraph($average, $width, $color){

	$finalWidth = (int) ($width * $average);
?> 

	<div style="width:<?php echo $finalWidth; ?>px; height:25px; background-color:<?php echo $color; ?>;"
...
{
	if (isset($_SESSION['dss_error']) && count($_SESSION['dss_error'])) {
		$numError = count($_SESSION['dss_error']);
		
		echo '<table id="errorMessage" width="550" align="center" cellpadding="20" cellspacing="0"><tr><td>';
		for ($i = 0; $i < $numError; $i++) {
			echo '&#8226; ' . $_SESSION['dss_error'][$i] . "<br>\r\n";
		}
		echo '</td></tr></table>';
