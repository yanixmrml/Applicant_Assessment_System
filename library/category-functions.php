...
function formatCategories($categories, $parentId)
{
	...
	foreach ($categories as $category) {
		if ($category['skill_parent_id'] == $parentId) {
			$navCat[] = $category;
		}
		
		// save the ids for later use
		$ids[$category['skill_id']] = $category;
	}	
	...
	while ($tempParentId != 0) {
		...
		$currentId = $parent[0]['skill_id'];

		// get all categories on the same level as the parent
		$tempParentId = $ids[$tempParentId]['skill_parent_id'];
		foreach ($categories as $category) {
		    // found one category on the same level as parent
			// put in $parent if it's not already in it
			if ($category['skill_parent_id'] == $tempParentId && !in_array($category, $parent)) {
				$parent[] = $category;
			}
		}
		...
		for ($i = 0; $i < $n; $i++) {
			$navCat2[] = $parent[$i];
			if ($parent[$i]['skill_id'] == $currentId) {
...
function getChildCategories($categories, $id, $recursive = true)
{
	...
	$child = array();
	for ($i = 0; $i < $n; $i++) {
		$catId    = $categories[$i]['skill_id'];
		$parentId = $categories[$i]['skill_parent_id'];
		if ($parentId == $id) {
			$child[] = $catId;
			if ($recursive) {
...
function fetchCategories()
{
    $sql = "SELECT skill_id, skill_name, skill_description
	        FROM skill
			WHERE skill_status = 'Activate' AND skill_parent_id = 0
			ORDER BY skill_id, skill_parent_id ";
    $result = dbQuery($sql);
